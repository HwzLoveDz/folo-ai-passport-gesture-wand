...
- 设备背面必须装有 **QMI8658A 六轴传感器**；没有该芯片时，手势录制与识别功能无法使用。
- 如果 PCB 已预留焊盘但未贴装 QMI8658A，可以使用低温锡膏自行补焊。务必确认芯片方向，并按照锡膏回流曲线与 QMI8658A 的温度限制控制风枪温度、风量和持续加热时间；温度过高或长时间直吹可能永久损坏传感器。焊接完成后应先检查虚焊、连锡和短路，再上电确认 I2C 通信。
